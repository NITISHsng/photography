...
const SearchTeams = () => {
  ...
  const { hiringRequest, handleChange } = useAppContext();
  const pathname = usePathname();

  const [userPath, setUserRole] = useState<string>("");
  useEffect(() => {
    if (!pathname) return;
    ...
    } else if (pathname.startsWith("/edit/client")) {
      setUserRole("client");
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
      ...
      <div className="space-y-4 max-h-[500px] overflow-y-auto">
        {filteredTeamMembers.map((member, index) => (
          <div
            key={index}
            className="bg-gradient-to-r from-gray-50 to-blue-50 dark:from-gray-700 dark:to-gray-600 rounded-xl border border-gray-200 dark:border-gray-600 hover:shadow-lg transition-all duration-300 p-4 pb-0"
          >
