...
const Services: React.FC<ServicesProps> = ({
  ...
  const handleTouchStart = (img: string, desc: string) => {
    // Start timer when user presses and holds
  ...
  const handleTouchEnd = () => {
    // Cancel timer if user lifts finger before threshold
    if (pressTimer) {
                        ...
                        setBookingData((prev) => {
                          ...
                          return {
                            ...
                            selectedService: alreadySelected
                              ? prev.selectedService.filter(
                                  (s) => s.id !== service.id
                                )
                              : [
                                  ...prev.selectedService,
                                  { id: service.id, price: finalPrice },
                                ],
                          };
                        })
                      }
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {preWeddingOptions.map((opt) => {
                    const isSelected =
                      bookingData.requiredServices.preWedding.some(
                        (s) => s.id === opt.id
                      );
                        ...
                        onClick={() =>
                          setBookingData((prev) => {
                            const alreadySelected =
                              prev.requiredServices.preWedding.some(
                                (s) => s.id === opt.id
                              );
                            ...
                            return {
                              ...
                              requiredServices: {
                                ...
                                preWedding: alreadySelected
                                  ? prev.requiredServices.preWedding.filter(
                                      (s) => s.id !== opt.id
                                    )
                                  : [
                                      ...prev.requiredServices.preWedding,
